...
void runSimulation (int elfAttackPower, char *map, int mapWidth, int mapHeight, int numUnits, const int maxUnits, int *unitIDs, 
                    int *targetIDs, int *numTargets, int *adjacentTargetIDs, int *numAdjacentTargets, 
                    position *openSquares, int *numOpenSquares, path_to_space *possiblePaths, int *numPossiblePaths,
                    int *numPathNodes, position *seenList, int *numSeen, int *queueMemory, 
                    path_node_queue *pathfindingQueue, int *round) 
{
    printf("current attack power: %d\n", elfAttackPower);
    while (true) {
        //printf("round %d\n", *round);
        ...
        for (int unitIndex = 0; unitIndex < numUnits; ++unitIndex) {
            ...
            if ((*numAdjacentTargets) != 0) {
                ...
                if (unitType == UNIT_TYPE_GOBLIN) {
                    ...
                    adjacentTarget->hitPoints -= elfAttackPower;
                }
                if (adjacentTarget->hitPoints <= 0) {
                    adjacentTarget->dead = true;
                    if (adjacentTarget->type == UNIT_TYPE_ELF) { return; }
                }
            }
        }
        ++(*round);
...
int main (int argc, char **argv) {
    ...
    map_unit *originalUnits = (map_unit *)malloc(maxUnits * sizeof(map_unit));
    ...
    // make a copy of the units so we can reset the state later
    memcpy(originalUnits, units, maxUnits * sizeof(map_unit));
    ...
    while (true) {
        runSimulation(elfAttackPower, map, mapWidth, mapHeight, numUnits, maxUnits, unitIDs, targetIDs, &numTargets,
                      adjacentTargetIDs, &numAdjacentTargets, openSquares, &numOpenSquares, possiblePaths, &numPossiblePaths,
                      &numPathNodes, seenList, &numSeen, queueMemory, &pathfindingQueue, &round);

        bool elfVictory = true;
        for (int i = 0; i < numUnits; ++i) {
            int unitID = unitIDs[i];
            map_unit *unit = &units[unitID];
            if (unit->dead) { 
        ...
        if (elfVictory) {
            ...
            memcpy(units, originalUnits, maxUnits * sizeof(map_unit));
            ++elfAttackPower;
            round = 0;
        }
    }
